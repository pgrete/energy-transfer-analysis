...
class EnergyTransfer:
    ...
    def forward(self, arr_in,arr_out):
        self.FFT.forward(np.ascontiguousarray(arr_in),np.ascontiguousarray(arr_out), heffte.scale.full)
    def backward(self, arr_in,arr_out,scale):
        self.FFT.backward(np.ascontiguousarray(arr_in),np.ascontiguousarray(arr_out))
    ...
    def getShellX(self,FTquant,Low,Up):
        ...
        if FTquant.shape[0] == 3:    
            ...
            for i in range(3):
                ...
                self.backward(tmp, Quant_X[i], heffte.scale.full)
                #print("but not here")
        else:
            ...
            self.backward(tmp, Quant_X, heffte.scale.full)        
